...
!$acc parallel loop collapse(1)
    do j = 1, n, 4
       do k = 1, n, 2
          do i = 1, n
             c(i,j+0) = c(i,j+0) + a(i,k+0)*b(k+0,j+0) + a(i,k+1)*b(k+1,j+0)
             c(i,j+1) = c(i,j+1) + a(i,k+0)*b(k+0,j+1) + a(i,k+1)*b(k+1,j+1)
             c(i,j+2) = c(i,j+2) + a(i,k+0)*b(k+0,j+2) + a(i,k+1)*b(k+1,j+2)
             c(i,j+3) = c(i,j+3) + a(i,k+0)*b(k+0,j+3) + a(i,k+1)*b(k+1,j+3)
          enddo
       enddo
    enddo
!$acc end parallel
